    ...
    with open(username_path, "r", encoding="UTF-8") as f:
        ...
        for row_content in res:
            ...
            # else:
                ...
                # Untag tweet content
                # if "TEST" in row_content[3].upper():
                #     row_content[3].replace(row_content[row_content[3].upper().index("TEST"):row_content[row_content[3].upper().index("TEST") + len("TEST")], """"<span style="color: yellow;">test</span>""")
                screen_name = row_content[1]
                ...
                <td><input type="checkbox" name= \"{name_id}\"  style="width:35px; height:35px"></td>
            </tr>""".format(number = counter, tweet_time = row_content[8], tweet_information = highlight_content, retweet_count = row_content[5], favorite_count = row_content[6], twitter_url = row_content[4], name_id = screen_name)
                counter += 1
    ...
    function validateForm() {
        ...
        for(var i = 0; i < lable_status.length; i ++){
            if(lable_status[i].checked){
                ...
                res_namelist.push(labelname)
            }
        }


    // console.log(res_namelist)
    exportCsv({
        title:res_namelist,
        titleForKey:res_namelist,
        data:[]
    });
    }
</script>
...
<div id="Demo" style="height: 67px; max-height: 989px;">
    <div id="Main">
        <form name="myForm" onsubmit="validateForm()" method="post">
            ...
            <th width= "74">full_text</th>
            <th width= "1">retweet_count</th>
            <th width= "1">favorite_count</th>
            <th width= "6">twitter_url</th>
            <th width="2">reslut</th>
            </tr>
            </tbody>""" + mid_content + """            
            </table>
            <br>
